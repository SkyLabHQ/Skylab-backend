...
contract BidTacToe is Initializable {
    ...
    */
    ...
    event BothRevealedBid(
        uint256 currentSelectedGrid,
        address[] grid,
        uint256 player1Balance,
        uint256 player2Balance,
        uint256 player1RevealedBids,
        uint256 player2RevealedBids,
        address winner
    );
    ...
    function revealBid(uint256 bid, uint256 salt) external onlyPlayers {
        ...
        if (gameStates[getOtherPlayer()] == 3) {
            ...
            if (existsOverallWinner()) {
                ...
                win(bidWinner, 10);
            } else if (balances[player1] == 0 && balances[player2] == 0) {
                if (occupiedGridCounts[player1] > occupiedGridCounts[player2]) {
                    win(player1, 10);
                }
                if (occupiedGridCounts[player1] < occupiedGridCounts[player2]) {
                    win(player2, 10);
            ...
            emit BothRevealedBid(
                currentSelectedGrid,
                grid,
                balances[player1],
                balances[player2],
                revealedBids[player1][currentSelectedGrid],
                revealedBids[player2][currentSelectedGrid],
                winner
            );
        }
    }
