...
void TimePerGrid_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void TimePerGrid_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 55;}

uint get_number_of_components() {return 3;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 47;}
...
ctx->componentMemory[coffset].templateName = "TimePerGrid";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "modLessThanScaler";
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 4]); // line circom 19
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 6]); // line circom 20
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 8],&signalValues[mySignalStart + 9]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[3],&circuitConstants[5],&signalValues[mySignalStart + 1]); // line circom 23
Fr_sub(&expaux[1],&signalValues[mySignalStart + 10],&expaux[3]); // line circom 23
Fr_pow(&expaux[0],&expaux[1],&circuitConstants[6]); // line circom 23
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 11]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_pow(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[6]); // line circom 25
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_idiv(&expaux[0],&signalValues[mySignalStart + 12],&signalValues[mySignalStart + 13]); // line circom 27
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mod(&expaux[0],&signalValues[mySignalStart + 12],&signalValues[mySignalStart + 13]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_mul(&expaux[3],&signalValues[mySignalStart + 14],&signalValues[mySignalStart + 13]); // line circom 29
Fr_add(&expaux[2],&expaux[3],&signalValues[mySignalStart + 15]); // line circom 29
Fr_eq(&expaux[0],&signalValues[mySignalStart + 12],&expaux[2]); // line circom 29
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[2]); // line circom 33
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template/function " << myTemplateName << " line 33. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
PFrElement aux_dest = &signalValues[mySignalStart + 16];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 14],&circuitConstants[7]); // line circom 42
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 7],&signalValues[mySignalStart + 16]); // line circom 44
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
TimePerGrid_2_create(1,0,ctx,"main",0);
TimePerGrid_2_run(0,ctx);
}
